      ...
      <Modal.Body>
        <Form id="category-create-form">
          ...
          <Form.Group className="mb-3">
            ...
            <Form.Control as="select" value={FormCategoryType} onChange={(ev) => setFormCategoryType(ev.target.value)}>
              <option></option>
              {
                Object.keys(category_type).map(k=>(
                  <option key={category_type[k]} value={category_type[k]}>{k}</option>
                ))
              }
              </Form.Control>
            {Error['name']}
          ...
          <div>
            <div>
              ...
              <div className="btn-container d-flex justify-content-between">
                ...
                <Button id = "category_filter_btn" type="submit" variant="secondary" onClick={filterToggle}>
                  <FaFilter />
              ...
              <tbody>
                {
                  Categories.map(category=>{
                    return (
                      <tr key={category.id}>
                        <td onClick={(ev)=>OpenEditModal(ev, category.id)}>{category.name}</td>
                        <td onClick={(ev)=>OpenEditModal(ev, category.id)}>{category.description}</td>
                        <td onClick={(ev)=>DeleteRecord(category.id)} width='5px'><FaRegTrashAlt /></td>
                      </tr>
                    )
                  })
                }
